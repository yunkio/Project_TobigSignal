...
class Junction:
    ...
    def step(self, action):
        ...
        # 차가 들어오는 경우
        self.cars_in([])
        prev = sum(sum((self.time - car[0]) ** 0.9 for car in road) for road in self.road)
    ...
    def get_reward(self, prev):
        if any(len(road) >= self.lim for road in self.road):
            return -1000, 1
        waiting = sum(sum((self.time - car[0]) ** 0.9 for car in road) for road in self.road)
        reward = prev - waiting
#         reward = 500-waiting
...
#         s = s[[0, 1, 3, 4, 6, 7, 9, 10], :].flatten()
#         a = np.zeros(1)
#         a[0] = self.signal
#         s = np.concatenate((s, a), axis=0)

        return s[[0, 1, 3, 4, 6, 7, 9, 10], :]
#         return s
