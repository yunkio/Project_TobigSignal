...
class DQNAgent():
    def __init__(self, action_size, frame_size=1, state_dict=None, target_update_interval=50):
    ...
    def get_action(self, x):
        ...
        if np.random.rand() <= self.eps:
            ...
            return int(torch.argmax(self.model(x.view(1, 80))))
    ...
    def append_sample(self, state, action, reward, next_state, done):
        state = torch.FloatTensor(self.preprocess_state(state)).view(80)
        next_state = torch.FloatTensor(self.preprocess_state(next_state)).view(80)
    ...
    def train_model(self):
        ...
        states = torch.zeros((self.batch_size, 80))
        next_states = torch.zeros((self.batch_size, 80))
